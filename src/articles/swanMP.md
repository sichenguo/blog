...
- 开发体验
  我是 18 年初的时候开始接触小程序，刚开始是做 PC 端开发，嗯，后来因为~~老大一声令下~~业务拓展的关系开始了小程序的 漫漫踩坑路，截止到现在小程序也做很多。其中的有在上家单位开发的企鹅洗衣小程序，还有目前在维护的青团社兼职小程序（偷偷打个广告）。emm,小程序里面常见的功能基本上都是有过一些了解的,怎么也算是~~踩坑~~经验丰富了吧 🤣。
  嗯，言归正传，对于开发者而言，开发体验真的是绕不过去的部分了，总体来说支付宝和微信小程序的开发体验还是挺好的，除了因为微信小程序后续变更的几个 API 导致加了几次班并且顺便顺便被测试妹子提了几个线上 bug，以及纠结了好几个日夜解决支付宝的表单组件的样式以及兼容性问题（尤其是在华为和小米手机上面，真的是血泪教训）等等。
  鉴于单位最近有可能再将业务拓展到百度系小程序这一块，提前体验一下百度小程序。以下部分仅代表个人观点~~如有错漏之处，请大家手下留情，评论区指出即可。🤔~~。
  - IDE 部分
  ...
  - 自定义组件
    不支持组件的框架肯定是噩梦（本人有幸经历过这段黑暗时期，虽然也有一些 trick 可以解决，但是毕竟还是会被恶心到），所幸目前微信、支付宝、百度小程序都是支持组件的，下面介绍下百度小程序的组件部分：
  ...
  - 其他一些实用/有趣的部分
    ...
    - **文件作用域** 支持局部定义域
    - **登录流程** 在用户已登录宿主账号的情况下整个登录流程可以完全静默完成，不需要用户**手动点击**、**手动点击**、**手动点击**。
    - **游客账号** 官方对于这一点的解答是这样的 “在未登录的时候，将数据与用户的 SwanID 关联，当用户登录后，通过 SwanID 找到用户登录前的数据，然后与 OpenID 关联，比如常见的场景例如电商类小 程序对购物车数据的同步。。”我所能想到的场景是方便实现游客登录这一个功能，不需要强制用户登录就可以体验一些功能，可以减少刚开始就授权登录而导致的一些用户流失。
- 学习成本
  对于 vue 熟练使用者，基本上学习成本就是熟悉 API 吧，emmm；对于 react 技术栈的开发者，难度也不是很大，可以采用这种 react 风格的第三方框架，例如 taro； 对于原本就是使用小程序第三方框架开发者（taro/mpvue/Okam 等），就更不用担心，因为这些是框架开发者需要考虑的事情（逃~）。

- 开发成本
  因为小程序本身就在 web 技术体系内，如果之就有过微信小程序开发经验，那么看看文档就可以动手开发了（毕竟 三家小程序的 API 风格极其相似，你懂得），如果没有小程序开发经验，问题也不大，毕竟小程序也是采用 html + js + css “三驾马车”的方式，只不过是需要学习一种新的类 Vue 语法。顺带一提，百度智能学院里面还是有不好技术干货的，可以帮助刚开始接触小程序的同学少走不少弯路。
- 项目迁移难度
